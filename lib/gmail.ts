...
export async function exchangeCodeForTokens(userId: string, code: string) {
  ...
  if (!tokens.access_token || !tokens.refresh_token) {
    ...
    const existing = await prismadb.gmail_Tokens.findFirst({ where: { user: userId, provider: "google" } });

    // Decrypt existing refresh token if needed for fallback
    const existingRefreshToken = existing ? decrypt(existing.refresh_token) : "";
    const existingAccessToken = existing ? decrypt(existing.access_token) : "";

    await prismadb.gmail_Tokens.upsert({
      where: { id: existing?.id || "new" },
      update: {
        access_token: encrypt(tokens.access_token || existingAccessToken || ""),
        refresh_token: encrypt(tokens.refresh_token || existingRefreshToken || ""),
...
export async function getGmailClientForUser(userId: string) {
  ...
  oauth2.setCredentials({
    access_token: decrypt(token.access_token) || undefined,
    refresh_token: decrypt(token.refresh_token) || undefined,
  ...
  try {
    ...
    // Compare new token with DECRYPTED old token
    if (newTokens?.token && newTokens.token !== decrypt(token.access_token)) {
      await prismadb.gmail_Tokens.update({
        where: { id: token.id },
        data: { access_token: encrypt(newTokens.token), updatedAt: new Date() } as any,
      });
    }
  } catch {
