...
export async function getAiClient(teamId: string) {
    ...
    if (teamConfig && !teamConfig.useSystemKey && teamConfig.apiKey) {
        ...
        const rawKey = systemConfig?.apiKey || process.env[`${modelRecord.provider}_API_KEY`];
        apiKey = rawKey ? decryptApiKey(rawKey) : null;
    ...
    switch (modelRecord.provider) {
        ...
        case "AWS":
            const aws = createAmazonBedrock({
                region: providerConfig.region || process.env.AWS_REGION || "us-east-1",
                accessKeyId: providerConfig.accessKeyId || process.env.AWS_ACCESS_KEY_ID,
                secretAccessKey: apiKey || process.env.AWS_SECRET_ACCESS_KEY,
            });
            model = aws(modelRecord.modelId);
            break;
