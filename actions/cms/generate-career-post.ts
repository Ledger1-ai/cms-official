    ...
    try {
        const { object } = await generateObject({
            ...
            schema: z.object({
                ...
                summary: z.string().describe("A brief summary of the role (2-3 sentences)."),
        ...
        const status = error?.status || error?.statusCode || "Unknown Status";
        const message = error?.message || "Unknown Error";
        const body = error?.body ? JSON.stringify(error.body) : "";
        throw new Error(`Failed to generate career post: ${message} (Status: ${status}) ${body}`);
    }
}
