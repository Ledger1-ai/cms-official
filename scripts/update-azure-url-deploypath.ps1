...
# Update env vars to use deploy path URL (fix 404 when connecting Azure Realtime)
$target = $containers[0]
foreach ($env in $target.env) {
  switch ($env.name) {
    'AZURE_OPENAI_REALTIME_WS_URL' { $env.value = 'wss://skyne-m9q617jz-swedencentral.cognitiveservices.azure.com/openai/deployments/gpt-realtime/realtime' }
...
$rev = "deploypath-" + (Get-Date).ToString('yyyyMMddHHmmss')
$bodyObj = [ordered]@{
  properties = [ordered]@{
    template = [ordered]@{
      revisionSuffix = $rev
...
Write-Host "PATCH body (deploypath) written to $temp"
